...
// function 0x00000344 mode=arm end=0x00000358
...
OVL_EXPORT void func_00000344(void) {
    if (gba_mod_function_entry(0x00000344u, 0u, &g_cpu)) return;
    if (g_runtime_fn_entry_hook) g_runtime_fn_entry_hook(0x00000344u);
L_00000344:
    /* 00000344  00000344 A strb r3,[r12,#0x301] */
    g_cpu.R[15] = 0x00000344u;
    if (runtime_should_yield()) return;
    if (g_runtime_insn_trace) runtime_insn_fp();
    uint32_t _cyc_00000344 = 1u;
    _cyc_00000344 = 1u;
    uint32_t _base_00000344 = g_cpu.R[12];
    uint32_t _off_00000344;
    _off_00000344 = 0x00000301u;
    uint32_t _ea_00000344 = _base_00000344 + _off_00000344;
    uint32_t _post_00000344 = _base_00000344 + _off_00000344;
    _cyc_00000344 += runtime_mem_cycles(_ea_00000344, 1u, 0u);
    runtime_trace_event(RUNTIME_TRACE_MEM_WRITE, 0x00000344u, _ea_00000344, (uint32_t)(g_cpu.R[3] & 0xFFu), 1u);
    bus_write_u8(_ea_00000344, (uint8_t)(g_cpu.R[3] & 0xFFu));
    g_cpu.R[15] = 0x00000348u;
    runtime_tick(_cyc_00000344);
    /* 00000348  00000348 A bl 0x00000358 */
    g_cpu.R[15] = 0x00000348u;
    if (runtime_should_yield()) return;
    if (g_runtime_insn_trace) runtime_insn_fp();
    uint32_t _cyc_00000348 = 1u;
    _cyc_00000348 = 3u;
    g_cpu.R[14] = 0x0000034Cu;
    g_cpu.R[15] = 0x00000358u;
    runtime_call_push_return(0x0000034Cu);
    runtime_tick(_cyc_00000348);
    _cyc_00000348 = 0u;
    runtime_dispatch(0x00000358u);
    if (g_cpu.R[15] != 0x0000034Cu) { runtime_call_cancel_return(0x0000034Cu); return; }
    ...
    if (arm_cond_passes(0x0u)) {
        _cyc_0000034C = 3u;
        g_cpu.R[15] = 0x00000344u;
        runtime_trace_event(RUNTIME_TRACE_BRANCH, 0x0000034Cu, 0x00000344u, 0u, 0u);
